"""
Graph Neural Network (Base) for Anti-Money Laundering Detection
Uses edge classification with only 4 base features (no payment type).
"""
...
def load_data():
    """Load preprocessed graph data with only base 4 edge features."""
    print("Loading GNN data (base 4 features)...")
    ...
    # Use only first 4 edge features (amount, hour, is_cross_border, is_currency_exchange)
    edge_features_full = gnn_data['edge_features']
    edge_features = torch.FloatTensor(edge_features_full[:, :4])
    ...
    print(f"Edge features: {edge_features.shape[1]} (base only, no payment type)")
...
def save_results(model, train_metrics, test_metrics):
    ...
    np.savez_compressed(
        os.path.join(RESULTS_DIR, 'gnn_base_results.npz'),
        # Test metrics
    ...
    print(f"Results saved to {RESULTS_DIR}/gnn_base_results.npz")
...
def main():
    ...
    print("=" * 60)
    print("GNN (BASE) FOR AML DETECTION")
    print("=" * 60)
